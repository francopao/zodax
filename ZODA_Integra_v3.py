...
def calcular_multiplos(ticker: str, datos_bbg: dict,
    ...
    def bbg_trim(col: str) -> pd.Series:
        """Convierte una columna diaria de BBG a frecuencia trimestral."""
        s = datos_bbg[ticker][col].reset_index()
        s["Fecha"] = s["Fecha"].dt.to_period("Q")
        return s.set_index("Fecha").squeeze()

    def ciq_trim(col: str) -> pd.Series:
        """Convierte una columna trimestral de CIQ al mismo formato de periodo."""
        s = datos_ciq[ticker][col].reset_index()
        s["Fecha"] = s["Fecha"].dt.to_period("Q")
        return s.set_index("Fecha").squeeze()
...
def calcular_z_final(multiplos_empresa: pd.DataFrame,
    ...
    primera_fecha = z_por_ventana[ANIOS_Z[idx_max]].dropna(how="all").index[0]
...
def calcular_basket(lista_comparables: list, todos_multiplos: dict,
                     market_cap: pd.DataFrame, fechas: pd.Index) -> pd.DataFrame:
    """
    Calcula el múltiplo promedio del grupo de empresas comparables (basket),
    ponderado por Market Cap de cada empresa.

    ¿Para qué sirve?
        Permite comparar la valorización de una empresa vs su sector.
        Por ejemplo: ¿está BAP cara o barata respecto a los bancos de la región?

    Proceso:
        1. Para cada empresa del basket, extrae sus 8 múltiplos.
        2. Calcula el peso de cada empresa = su MktCap / MktCap total del basket.
        3. Multiplica múltiplo × peso para cada empresa y suma → múltiplo del basket.

    Nota: el basket se calcula dentro del loop (recalculando pesos en cada iteración)
          para que los pesos se actualicen con datos nuevos automáticamente.
    """
    nombres_mult = [
    ...
    for ticker in lista_comparables:
        for mult in nombres_mult:
            temp = pd.DataFrame(todos_multiplos[ticker][mult])
            temp.rename(columns={mult: ticker}, inplace=True)
            dfs[mult] = pd.concat([dfs[mult], temp], axis=1)

    # Pesos basados en Market Cap (se recalculan diario)
    mc = market_cap[lista_comparables]
    pesos = mc.div(mc.sum(axis=1), axis=0)

    resultado = pd.DataFrame(index=fechas)
    for mult in nombres_mult:
        resultado[mult] = pesos.multiply(dfs[mult]).sum(axis=1)
...
def main():
    ...
    mapa_hojas = (
        {t: datos_financials for t in todas_finan}
        | {t: datos_cons_const for t in todas_cons_const}
        | {t: datos_min_utl    for t in todas_min_utl}
    )
